...
client.on('message', message => {//Toxic Codes
  ...
  if (message.content.startsWith('^se1t1s')) {
...
client2.on('ready', () => console.log('I am ready!'));

client2.on('disconnect', () => console.log('I disconnected!'));

client2.on('reconnecting', () => console.log('I am disconnecting!'));

client2.on('voiceStateUpdate', (oldMember, newMember) => {
  let newUserChannel = newMember.voiceChannel
  let oldUserChannel = oldMember.voiceChannel
  const serverQueue = queue.get(oldMember.guild.id);
  ...
  if(oldUserChannel === undefined && newUserChannel !== undefined) {
    ...
    // User leaves a voice channel
      if(oldMember.id === '498378677512437762'){
          ...
          if(client2.guilds.get(oldMember.guild.id).voiceConnection != null){
              if(client2.guilds.get(oldMember.guild.id).voiceConnection.channel.id === oldUserChannel.id){
                    if(oldUserChannel.members.size < 2){
                        serverQueue.songs = [];
                        serverQueue.connection.dispatcher.end('No members left in the channel!')
                    }    
...
client2.on('message', message => {//Toxic Codes
    var argresult = message.content.split(` `).slice(1).join(' ');//Toxic Codes
      if (!developers.includes(message.author.id)) return;
  ...
  if (message.content.startsWith('^setw')) {
  client2.user.setActivity(argresult, {type:'WATCHING'});//Toxic Codes
      message.channel.send(`**✅   ${argresult}**`)//Toxic Codes
  } else 
  if (message.content.startsWith('^setl')) {
  ...
  if (message.content.startsWith('^se1t1s')) {
    client2.user.setGame(argresult, "https://www.twitch.tv/zero");
      message.channel.send(`**✅**`)//Toxic Codes
  }
  if (message.content.startsWith('^setname')) {
  ...
  if (message.content.startsWith('^setprefix')) {//Toxic Codes
  client2.user.setPrefix(argresult).then
      message.channel.send(`Changing Prefix ..**${argresult}** `)//Toxic Codes
} else
if (message.content.startsWith('^setavatar')) {//Toxic Codes
